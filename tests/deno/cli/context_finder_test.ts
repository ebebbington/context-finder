...
Deno.test({
    ...
    async fn(): Promise<void> {
        const p = await Deno.run({
            cmd: ['deno', 'run', '--allow-read', '--allow-run', '../../../mod.ts', '../read.conf', 'write.txt'],
            cwd: "./tests/deno/cli"
        })
        const status = await p.status()
        p.close()
...
Deno.test({
    ...
    async fn(): Promise<void> {
        const p = await Deno.run({
            cmd: ['deno', 'run', '--allow-read', '--allow-run', '../../../mod.ts', '../read.conf', 'write.txt', 'idontexist'],
            cwd: "./tests/deno/cli"
        })
        const status = await p.status()
        p.close()
...
Deno.test({
    ...
    async fn(): Promise<void> {
        const p = await Deno.run({
            cmd: ['deno', 'run', '--allow-read', '--allow-run', '--allow-write', '../../../mod.ts', '../read.conf', 'write.conf', 'version-1'],
            cwd: "./tests/deno/cli"
        })
        const status = await p.status()
        p.close()
        ...
        const writeFileContent = decoder.decode(Deno.readFileSync("tests/deno/cli/write.conf"))
